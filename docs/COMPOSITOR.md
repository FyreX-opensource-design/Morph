...
- **Keybind config** (INI-style): repeated `[bind]` sections with `mods`, `key`, `action`, optional `command` (for `exec`), optional `when` (shell predicate, evaluated **on each key press**; exit 0 = bind active). Optional **`[hooks]`** section: **`startup`**, **`shutdown`**, **`reload`** shell strings (`sh -c`). See **`docs/CONFIG.md`** for full syntax and every action; **`morph.conf.example`** is a commented starter file. Modifiers and keysym are read **before** `wlr_keyboard_notify_key()` so XKB state matches user chord; `exec` / `when` / hooks use `posix_spawnp("sh", …)` instead of `fork` for safer interaction with wlroots’ threads.
  - **Actions** (summary): `quit`, `close`, `exec`, layout toggles, window focus cycling (`nextWindow` / `prevWindow`), tile **sort** moves (`tile_left` / `tile_right`, `tile_first` / `tile_last`, `tile_move`), tile **grid** moves (`tile_up` / `tile_down`, column top/bottom, `tile_grid_move` with `command=`). Details and `command=` rules are in **`docs/CONFIG.md`**. Built-in defaults include **Super+t** for `layout_toggle` when no config file is found, the file cannot be read, or it defines no binds.
...
- **Pointer**: `Super+drag` moves a window (unchanged; not yet in config file). Optional **`[focus]`** policy (`ClickToFocus` default, `FocusFollowsMouse`, `SloppyFocus`) — see **`docs/CONFIG.md`**.
- **Window focus cycling**: `server->focus_order` is an **MRU** list over the same toplevels as `server->toplevels`, newest first. Windows join it at the **tail** on creation, leave it on destroy, and are promoted to the **head** by every successful `focus_toplevel()` handoff (click, map, workspace switch, foreign-toplevel and xdg-activation requests). Eligibility is evaluated **live** at cycle time (`toplevel_focus_candidate`: mapped, initialized, non-minimized, on the current workspace), so minimize/restore and workspace moves need no extra bookkeeping. Workspace switching also picks its focus target from this list, so returning to a workspace restores the window last used there. Keybinds: `nextWindow` / `prevWindow`; IPC: `window focus next|prev`; CLI: `--window-focus next|prev`.
  - **Held-modifier sessions**: when the bind carries a modifier, `server_window_cycle_step()` opens a session (`comp_server.cycle`) that **snapshots** the candidate ring once and walks it on each press, so holding `Alt` and tapping `Tab` traverses the whole list. Stepping calls `focus_toplevel()` with `cycle.suppress_mru` set so the preview does not reorder history underneath the frozen ring; `keyboard_handle_modifiers()` commits the selection to the MRU head once the held bits leave `modifiers.depressed`, and `Esc` is intercepted in `keyboard_handle_key()` to cancel back to `cycle.origin`. `Shift` is masked out of the held bits so forward and backward binds share one session. Destroyed windows are spliced out of the ring by `window_cycle_forget()`, and workspace changes commit the session because candidates are workspace-scoped. Binds without a modifier, plus the IPC and CLI paths, fall back to the immediate single-shot cycle. While held, `window_cycle_overlay_sync()` presents a compositor-owned list (`src/ui.c`) on `server->ui_tree` (above client overlay surfaces) with title / app_id rows and the current candidate highlighted; the overlay rejects pointer input and is dismissed on commit or cancel.
...
| **Scene** | Root scene → layer trees (background, windows, client overlays) then `ui_tree` for compositor-owned chrome such as the Alt-Tab switcher. Stacking = z-order among window nodes. |
...
**Build deps:** wlroots 0.20.2+, `wayland-server`, `wayland-protocols` (for `xdg-shell` codegen), `xkbcommon`, plus wlroots’ normal graphics stack (DRM, libinput, etc.) for a DRM session.
